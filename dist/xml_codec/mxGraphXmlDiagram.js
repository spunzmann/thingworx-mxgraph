...
export function createGraphFromXML(container, data, customShapes, layout, edgeStyle) {
    ...
    var xmlData = mxUtils.parseXml(data);
    var decoder = new mxCodec(xmlData);
    var node = xmlData.documentElement;
    ...
    var defaultStyles = require("../resources/defaultStyles.xml");
...
var layoutExecutorFactory = {
    "Horizontal Flow": function (graph) {
        var layout = new mxHierarchicalLayout(graph, mxConstants.DIRECTION_WEST);
        layout.execute(graph.getDefaultParent(), null);
    },
    "Vertical Flow": function (graph) {
        var layout = new mxHierarchicalLayout(graph, mxConstants.DIRECTION_NORTH);
        layout.execute(graph.getDefaultParent(), null);
    },
    "Horizontal Tree": function (graph) {
        var layout = new mxCompactTreeLayout(graph, true);
        layout.edgeRouting = false;
        layout.levelDistance = 30;
        layout.execute(graph.getDefaultParent());
    },
    "Vertical Tree": function (graph) {
        var layout = new mxCompactTreeLayout(graph, false);
        layout.edgeRouting = false;
        layout.levelDistance = 30;
        layout.execute(graph.getDefaultParent());
    },
    "Radial Tree": function (graph) {
        var layout = new mxRadialTreeLayout(graph, false);
        layout.autoRadius = true;
        layout.levelDistance = 80;
        layout.execute(graph.getDefaultParent());
    },
    "Organic": function (graph) {
        var layout = new mxFastOrganicLayout(graph);
        layout.execute(graph.getDefaultParent());
    },
    "Circle": function (graph) {
...
function decompress(data) {
    if (data == null || data.length == 0 || typeof (pako) === 'undefined') {
        ...
        return zapGremlins(decodeURIComponent(bytesToString(pako.inflateRaw(atob(data)))));
    }
}
/**
...
//# sourceMappingURL=mxGraphXmlDiagram.js.map